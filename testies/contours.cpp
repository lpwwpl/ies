...
void MarchingSquares::levelPaths::openPoligon(double x, double y) {
    _current.push_back(QPointF(x, y));

}

void MarchingSquares::levelPaths::lineTo(double x, double y) {
    _current.push_back(QPointF(x, y));

}

void MarchingSquares::levelPaths::closePoligon() {
    if (_current.size() > 0) {
...
MarchingSquares::IsoCell::IsoCell() {
    _flipped = false;
}

MarchingSquares::IsoCell::IsoCell(int nInfo, bool isFlipped) {
    _neighborInfo = nInfo;
    _flipped = isFlipped;
}


void MarchingSquares::IsoCell::setFlipped(bool isFlipped) {
    _flipped = isFlipped;


}
void MarchingSquares::IsoCell::setNeighborInfo(int nInfo) {
    _neighborInfo = nInfo;

}

int MarchingSquares::IsoCell::getNeighborInfo() {
    return _neighborInfo;

}

bool MarchingSquares::IsoCell::isFlipped() {
    return _flipped;
}
...
/**
     ...
     */
QPointF MarchingSquares::IsoCell::normalizedPointCCW(side cellSide) {
    switch (cellSide) {
    case BOTTOM:
...
/**
     ...
     */
MarchingSquares::IsoCell::side MarchingSquares::IsoCell::firstSideCCW(side prev) {
    ...
    case 10:
        ...
        case side::TOP:
            return side::BOTTOM;
        default:
            cout << ".firstSideCCW: case 10!" << endl;
        }
    default:
        cout << ".firstSideCCW: default!" << endl;
    }
    return side::NONE;
}
...
/**
     ...
     */
MarchingSquares::IsoCell::side MarchingSquares::IsoCell::secondSideCCW(side prev) {
    ...
    case 5:
        ...
        default:
            cout << ".secondSideCCW: case 5!" << endl;
        }
    case 10:
        switch (prev) {
        ...
        default:
            cout << ".secondSideCCW: case 10!" << endl;
        }
    default:
        cout << ".secondSideCCW: shouldn't be here!  Neighborinfo = " << _neighborInfo << endl;
        return side::NONE;
    }
}
...
MarchingSquares::IsoCell::side MarchingSquares::IsoCell::nextCellCCW(side prev) {
...
/**
     ...
     * @param prev
     */
void MarchingSquares::IsoCell::clearIso() {
    switch (_neighborInfo) {
    case 0:
    case 5:
    case 10:
...
/**
     ...
     */
double MarchingSquares::IsoCell::getLeft() {
    return _left;
}
...
/**
     ...
     */
void MarchingSquares::IsoCell::setLeft(double left) {
    _left = left;
}
...
/**
     ...
     */
double MarchingSquares::IsoCell::getRight() {
    return _right;
}
...
/**
     ...
     */
void MarchingSquares::IsoCell::setRight(double right) {
    _right = right;
}
...
/**
     ...
     */
double MarchingSquares::IsoCell::getTop() {
    return _top;
}
...
/**
     ...
     */
void MarchingSquares::IsoCell::setTop(double top) {
    _top = top;
}
...
/**
     ...
     */
double MarchingSquares::IsoCell::getBottom() {
    return _bottom;
}

MarchingSquares::MarchingSquares(vector<vector<double> >& data, double xMin, double xMax, double yMin, double yMax) {
    _xMin = xMin;
    _xMax = xMax;
    _yMin = yMin;
    _yMax = yMax;
...
/**
     ...
     */
void MarchingSquares::IsoCell::setBottom(double bottom) {
    _bottom = bottom;
}
/**
...
vector<MarchingSquares::levelPaths> MarchingSquares::mkIsos(vector<double>& levels) {
    // Pad data to guarantee iso GeneralPaths will be closed shapes.

    double superMin = levels[0];
    for (int i = 1; i < levels.size(); i++) {
        superMin = min(superMin, levels[i]);
    }
    superMin--;
    ...
    vector<levelPaths> isos;
    IsoCell* contour;
    for (int i = 0; i < levels.size(); i++) {
        // Create contour for this level using Marching Squares algorithm.
        int rows, cols;
        contour = mkContour(levels[i], rows, cols);
        // Convert contour to GeneralPath.
        isos.push_back(mkIso(contour, levels[i], rows, cols));
...
MarchingSquares::IsoCell* MarchingSquares::mkContour(double& level, int& numRows, int& numCols) {
        ...
        for (int c = 0; c < numCols; c++) {
            ...
            if (nInfo == 5 || nInfo == 10) {
                ...
                double center = (dataP[r * p_cols + c] + dataP[r * p_cols + c + 1]
                    + dataP[(r + 1) * p_cols + c + 1] + dataP[(r + 1) * p_cols + c]) / 4;
...
/**
     ...
     */
     /* private */
MarchingSquares::levelPaths MarchingSquares::mkIso(IsoCell* isoData, double threshold, int& numRows, int& numCols) {
    int r, c;
    for (r = 0; r < numRows; r++) {
        for (c = 0; c < numCols; c++) {
...
void MarchingSquares::isoSubpath(IsoCell* input, int r, int c, levelPaths& iso, int& numRows, int& numCols) {
...
double* MarchingSquares::padData(vector<vector<double> >& data) {
    ...
    for (int i = 0; i < rows; i++) {
        for (int j = 0; j < cols; j++) {
            padded[(i + 1) * p_cols + j + 1] = data[i][j];
        }
    }
    return padded;
}
...
void MarchingSquares::interpolateCrossing(IsoCell* isoData, int r, int c, double threshold, int rows, int cols) {
